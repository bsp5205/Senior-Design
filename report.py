...
class Submission_File:
    def __init__(self, code, name, scores):
        self.code = code
        self.name = name
        #self.id = id   <-- no need for id, we have the file
...
def create_file(code, name, id, scores):
    file = Submission_File(code, name, id, scores)
    return file
...
language = "java"
file_1 = create_file(code, 'file1', scores_1)
file_2 = create_file('test2', 'file2', scores_1)
...
file_3 = create_file('test3', 'file3', scores_1)
file_4 = create_file('test4', 'file4', scores_1)
...
student_3 = create_student('Student 2', '123', submission_2)

student_4 = create_student('Student 2', '123', submission_2)
...
t = [50, 50, 0, 50, 50, 50, 50, 50, 100, 50, 50, 50, 50, 50]
threshold_list = t
            ...
            with a.div(klass='subheadContent subheadContent--flex-end'):
                with a.div(klass='studentSelection'):
                    with a.form(action='http://localhost:3000/next_student', method='post'):
                        ...
                        with a.button(klass='Button Button--icon-action gradebookMoveToNext prev', id='prev-student-button',
                    ...
                    with a.form(action='http://localhost:3000/next_student', method='post'):
                        ...
                        with a.button(klass='Button Button--icon-action gradebookMoveToNext next', id='next-student-button',
                                    ...
                                    for j in range(len(student_list[i].submission.file_list)):
                                        with a.tr():
                                            ...
                                            for k in range(len(student_list[i].submission.file_list[j].complexity_scores)):
                                                a.td(klass='score',
                                                     _t=student_list[i].submission.file_list[j].complexity_scores[k])
                                            for k in range(len(student_list[i].submission.file_list[j].coupling_scores)):
                                                a.td(klass='score',
                                                     _t=student_list[i].submission.file_list[j].coupling_scores[k])
                                            for k in range(len(student_list[i].submission.file_list[j].cohesion_scores)):
                                                a.td(klass='score',
                                                     _t=student_list[i].submission.file_list[j].cohesion_scores[k])
                    with a.div(id='inner-grid-summary'):
                        with a.div():
                            with a.table():
                                with a.tr():
                                    ...
                                    for j in range(len(student_list[i].submission.file_list)):
                                        with a.tr():
                                            ...
                                            for k in range(len(student_list[i].submission.file_list[j].general_scores)):
                                                a.td(klass='score',
                                                     _t=student_list[i].submission.file_list[j].general_scores[k])
                        ...
                        with a.table(klass='threshold-table'):
                            for i in range(len(qma_metric_score_list)):
                                ...
                                for j in range(len(qma_metric_score_list[i].metric_scores)):
                                    with a.tr():
                                        a.td(klass='threshold-table-content', _t=qma_metric_score_list[i].metric_names[j])
                                        with a.td():
                                            with a.div(klass='slider qma-threshold-update'):
                                                a.input(klass='s' + str(counter + 1), id='rangeInput', max='100', min='0',
                        ...
                        with a.table(klass='threshold-table'):
                            for i in range(len(cma_metric_score_list)):
                                ...
                                for j in range(len(cma_metric_score_list[i].metric_scores)):
                                    with a.tr():
                                        a.td(klass='threshold-table-content', _t=cma_metric_score_list[i].metric_names[j])
                                        with a.td():
                                            with a.div(klass='slider qma-threshold-update'):
                                                a.input(klass='s' + str(counter + 1), id='rangeInput', max='100', min='0',
                                                        name='rangeInput', onchange='updateTextInput(this.parentNode);',
                            ...
                            for j in range(len(qma_metric_score_list[i].metric_scores)):
                                with a.tr():
                                    with a.td(klass='table-title '+qma_metric_score_list[i].instance_name):
                                        ...
                                        a.p(klass='metric-info-popup', _t=qma_metric_score_list[i].metric_description[j])
                    ...
                    with a.table(klass='cma-score-table'):
                        for i in range(len(cma_metric_score_list)):
                            ...
                            for j in range(len(cma_metric_score_list[i].metric_scores)):
                                with a.tr():
                                    with a.td(klass='table-title '+cma_metric_score_list[i].instance_name):
                                        ...
                                        with a.svg(klass='bi bi-question-circle question', fill='currentColor', height='16',
                                            ...
                                            a.path(
                                                d='M5.255 5.786a.237.237 0 0 0 .241.247h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286zm1.557 5.763c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z')
                                        a.p(klass='metric-info-popup', _t=cma_metric_score_list[i].metric_description[j])
                                    with a.td(klass='progress-bar-cont'):
                                        a.div(klass='color-circle s' + str(counter + 1), role='progressbar',
                                ...
                                counter += 1
                    with a.div():
                        a.span(klass='grid-title', _t='Metric Analysis')
                    with a.button(klass='pie_chart_button', onclick='showPiePopup()', style='width: 90%; height: 40%; margin-left: 5%'):
                        a.div(id='chartdiv')
                                            ...
                                            with a.div(klass='container'):
                                                with a.form(action='http://localhost:3000/change_file',
                                                    ...
                                                    a.button(klass='submission_button',
                                                             name=str(selected_student.submission.file_list[i].id) + '-' + str(selected_student.studentID), type='submit', _t=selected_student.submission.file_list[i].name)
                                                    with a.label():
                                                    ...
                                                    for j in range(len(qma_metric_score_list)):
                                                        for l in range(len(qma_metric_score_list[j].metric_scores)):
                                                            with a.label():
                                                                a.input(klass='s' + str(counter + 1) + ' change_file_secret file_threshold', name='fuck', value=threshold_list[counter])
                                                            counter += 1
                                                    for j in range(len(cma_metric_score_list)):
                                                        for l in range(len(cma_metric_score_list[j].metric_scores)):
                                                            with a.label():
                                                                a.input(klass='s' + str(counter + 1) + ' change_file_secret file_threshold', name='fuck', value=threshold_list[counter])
                                                            counter += 1
                                                    with a.a(klass='submission-file-download icon-download float-right', download='', href='/courses/2173881/assignments/13793262/submissions/6962095?download=132327340'):
