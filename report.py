                            ...
                            for j in range(len(qma_metric_score_list[i].metric_scores)):
                                with a.tr():
                                    with a.td(klass='table-title ' + qma_metric_score_list[i].instance_name):
                                        with a.svg(klass='bi bi-question-circle question', fill='currentColor',height='16',viewbox='0 0 16 16', width='16', xmlns='http://www.w3.org/2000/svg'):
                                            a.path(d='M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z')
                                            a.path(d='M5.255 5.786a.237.237 0 0 0 .241.247h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286zm1.557 5.763c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z')
                                        a(qma_metric_score_list[i].metric_names[j])
                                        with a.div(klass='test'):
                                    ...
                                    for i in range(len(selected_student.submission.file_list)):
                                        with a.div(klass='submission-file'):
                                            with a.div(klass='container'):
                                                with a.form(action='http://127.0.0.1:5000/change_file', klass='file_form_test', id='file_form', method='post'):
                                                    with a.span():
                                                        a.button(klass='submission_button cutoff', name=str(selected_student.submission.file_list[i].id) + '-' + str(selected_student.studentID), type='submit', _t=selected_student.submission.file_list[i].name)
                                                        with a.a(klass='submission-file-download icon-download float-right', download='', href=selected_student.submission.file_list[i].link):
                                                            with a.span(klass='screenreader-only'):
                                                                with a.svg(klass='bi bi-download', fill='currentColor', height='16', viewbox='0 0 16 16', width='16', xmlns='http://www.w3.org/2000/svg'):
                                                                    a.path(d='M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z')
                                                                    a.path(d='M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z')
...
def air_file(student_info, assignment_info, report_metrics, file_stu):
    ...
    if mean(cc_avg) < 50:
        qma_observation.append('<b>CC:</b> The cyclomatic compexity is too high. Try modularizing duplicate code and shortening complex functions.')
    if mean(sloc_avg) < 50:
        qma_observation.append('<b>SLOC:</b> The line of code in students\' projects is too large. Try reducing the lines of code.')
    if mean(wmc_avg) < 50:
        qma_observation.append('<b>WMC:</b> Weighted methods per class is too high. Try reducing the number and length of methods in the code.')
    if mean(abc_avg) < 50:
        qma_observation.append('<b>ABC:</b> Compared to the size of the code, there are an excessive amount of assignments, branches, or conditionals. Try looking for places where unnecessary assignments, branches, or conditionals can be avoided.')

    if mean(cof_avg) < 50:
        qma_observation.append('<b>COF:</b> The coupling factor is too high. Try using more internal methods or attributes as opposed to coupled ones.')
    if mean(cbo_avg) < 50:
        qma_observation.append('<b>CBO:</b> The coupling between objects is too high. Try combining smaller classes that have a high number of coupled methods.')

    if mean(dit_avg) < 50:
        qma_observation.append('<b>DIT:</b> The maximum depth of inheritance of student\'s classes is too high. Try referencing external methods directly.')
    if mean(mhf_avg) < 50:
        qma_observation.append('<b>MHF:</b> There are an abnormal amount of methods declared as public. Try assigning methods only meant to be accessed in the parent class as private.')
    if mean(ahf_avg) < 50:
        qma_observation.append('<b>AHF:</b> There are an abnormal amount of variables declared as public. Try assigning variables only meant to be accessed in the parent class as private.')
    ...
    if mean(cp_avg) < 50:
        cma_observation.append('<b>CP:</b> There are not enough comments in the code. Adding additional descriptions of functionality is a good way to comment code.')
    if mean(tc_avg) < 50:
