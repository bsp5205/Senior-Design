...
# lists of literals for static info and to ensure the system can be changed easily
...
# metric descriptions --------------------------------------
complexity_metric_descriptions = [
    'A measure of the number of possible paths through a program\'s control flow graph. CC Determines the complexity and the potential difficulty of testing and maintaining the software. ',
...
coupling_metric_descriptions = [
    'A measure of the degree to which one module or component depends on another. It indicates the strength of the relationship between two modules.',
...
cohesion_metric_descriptions = [
    'A measure of the number of ancestor classes in a class hierarchy. It indicates how far a class is from the root of the inheritance tree. ',
...
qma_observation = [
    '<b>ABC:</b> <br>\tCompared to the size of the code, there are an excessive amount of assignmnets, branches, or conditionals. Try looking for places where unnessessary assignmnets, brances or conditionals can be avoided.',
    '<b>MHF</b>: <br>\tThere are an abnormal amount of methods declared as public. Try assigning methods only meant to be accessed in the parent class as private.',
    '<b>AHF:</b> <br>\tThere are an abnormal amount of variables declared as public. Try assigning variables only meant to be accessed in the parent class as private.']
cma_observation = []
...
def go_here(guy, fill, thresh):
    ...
    # Having 1k+ lines of JS in one line is awful, but everything needs to be inline for canvas. To see everything, copy/paste into a JS viewer
    with a.script():
    ...
    # Having 2k+ lines of CSS in one line is awful, but everything needs to be inline for canvas. To see everything, copy/paste into a CSS viewer
    with a.style():
                ...
                with a.div(klass='studentSelection'):
                    with a.form(action='http://127.0.0.1:5000/next_student', id='next_student_form', method='post'):
                        ...
                        with a.button(klass='Button Button--icon-action gradebookMoveToNext prev',
                                      id='prev-student-button', type='submit', **{'aria-label': 'Previous'}):
                        ...
                        with a.div(klass='summary-table'):
                            with a.table():
                                ...
                                with a.tr():
                                    ...
                                    for i in range(len(coupling_metric_names[i])):
                                        a.td(_t=coupling_metric_names[i][i])
                                ...
                                for i in range(len(student_list)):
                                    for j in range(len(student_list[i].submission.file_list)):
                                        with a.tr():
                                            ...
                                            for k in range(
                                                    len(student_list[i].submission.file_list[j].complexity_scores)):
                                                a.td(klass='score', _t=str(
                                                    student_list[i].submission.file_list[j].complexity_scores[k]))
                                            for k in range(
                                                    len(student_list[i].submission.file_list[j].coupling_scores)):
                                                a.td(klass='score',
                                                     _t=str(student_list[i].submission.file_list[j].coupling_scores[k]))
                                            for k in range(
                                                    len(student_list[i].submission.file_list[j].cohesion_scores)):
                                                a.td(klass='score',
                                                     _t=str(student_list[i].submission.file_list[j].cohesion_scores[k]))
                    with a.div(id='inner-grid-summary'):
                        with a.div():
                            with a.table():
                                with a.tr():
                                    ...
                                    for j in range(len(student_list[i].submission.file_list)):
                                        with a.tr():
                                            ...
                                            for k in range(len(student_list[i].submission.file_list[j].general_scores)):
                                                a.td(klass='score',
                                                     _t=str(student_list[i].submission.file_list[j].general_scores[k]))
                                            for k in range(len(student_list[i].submission.file_list[j].naming_scores)):
                                                a.td(klass='score',
                                                     _t=str(student_list[i].submission.file_list[j].naming_scores[k]))
                    with a.div(id='inner-grid-summary'):
                        with a.div():
                            with a.table():
                                with a.tr():
                                        ...
                                        with a.div(klass='submission-file'):
                                            with a.div(klass='container'):
                                                with a.form(action='http://127.0.0.1:5000/change_file',
                                                    ...
                                                    with a.span():
                                                        a.button(klass='submission_button cutoff', name=str(
                                                            selected_student.submission.file_list[i].id) + '-' + str(
                                                            selected_student.studentID), type='submit',
                                                                 _t=selected_student.submission.file_list[i].name)
                                                        with a.a(
                                                                klass='submission-file-download icon-download float-right',
                                                                download='',
...
def air_file(student_info, assignment_info, report_metrics, file_stu):
    ...
    for student in student_info:
        ...
        for file in file_stu[student[0]]:
            ...
            file_coh_metric = [report_metrics[file[0]]['DIT'], report_metrics[file[0]]['MHF'],
                               report_metrics[file[0]]['AHF']]
